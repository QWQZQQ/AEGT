...
import matplotlib
matplotlib.use('TkAgg')  # 使用 TkAgg 后端（或其他 GUI 后端，如 Qt5Agg）
...
class CARPTester:
    def __init__(self,
        ...
        if self.tester_params['test_data_load']['type'] == 'SyntheticData':
            self.test_data.SyntheticDataload(self.tester_params['test_data_load']['filename'])
        else:
            self.test_data.StandardDatasetload(self.tester_params['test_data_load']['filename'])
    ...
    def run(self):
        ...
        test_loader = DataLoader(self.test_data, batch_size, True, num_workers=0,generator = torch.Generator(device='cuda'))
