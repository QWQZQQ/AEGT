...
class CARPEnv:
    ...
    def load_problems(self, batch_size, batch):
        ...
        (depot, customer, customer_demand), self.graph_info, self.D, self.edge_distance = batch
    ...
    def _get_travel_distance(self):
        ...
        total_dhcost = torch.zeros(self.batch_size*self.pomo_size, device=self.device)
        ...
        f_1 = torch.zeros(num_samples, device=self.device)
        f_2 = torch.zeros(num_samples,  device=self.device)
        depot = torch.zeros(num_samples, device=self.device)
