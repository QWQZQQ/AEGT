...
class CARPDataset(Dataset):
    ...
    vertex_size  : int
        number of vertex to be generated in each training example
    edge_size  : int
    ...
    """
    def __init__(self, num_samples, vertex_size, edge_size, device, max_load=100, max_demand=10, max_dhcost=10, seed=None):
        ...
        features = torch.zeros((self.num_samples, edge_size + 1, 8), dtype=torch.float32)
        dynamic = torch.zeros((self.num_samples, edge_size + 1), dtype=torch.float32)
        graph_info_ori = torch.zeros((self.num_samples, edge_size + 1, 5))
        D_tensor = torch.zeros((self.num_samples, vertex_size, vertex_size))

        for sample in tqdm(range(num_samples), desc="Processing graphs"):
            # 按度生成图
            G, depot, total_cost, total_demand = generate_graph_degree_BFS(vertex_size, edge_size,
            ...
            features[sample, i, :] = torch.tensor(node_feature)
            dynamic[sample, i] = torch.tensor(dynamic_np)
            graph_info_ori[sample, i, :] = torch.tensor(graph_info_)

            i += 1
            for node_ori_1, node_ori_2, attributes in G.edges(data=True):
        ...
        edge_distance = torch.min(torch.min(distances_1, distances_2), torch.min(distances_3, distances_4))  # [num_samples ,edge_size+1, edge_size+1]

        graph_info_ori = graph_info_ori.permute(0, 2, 1)  # [num_samples , edge_size + 1, 5] --->  [num_samples ,5, edge_size + 1]

        self.depot = features[:, 0, :] # [num_samples, _, 8]
        self.customer = features[:, 1:, :] # [num_samples , edge_size, 8]
        self.graph_dynamic = dynamic[:,1:] # [num_samples, edge_size, 1]

        self.graph_info = graph_info_ori # [num_samples ,5, edge_size + 1]
        self.D = D_tensor  # 每张图的节点之间最短路径邻接矩阵  [num_samples ,vertex_size, vertex_size]

        self.edge_distance = edge_distance # [num_samples ,edge_size+1, edge_size+1]
    ...
    def __getitem__(self, idx):
        ...
        return ((self.depot[idx], self.customer[idx], self.graph_dynamic[idx]), self.graph_info[idx], self.D[idx], self.edge_distance[idx])
...
if __name__ == '__main__':
    train_data = CARPDataset(10000, 50, 100, 100, 10, 10,None)
